...
/*   Updated: 2021/03/02 18:50:19 by amoujane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft
{
	template <class T1, class T2> struct pair{
		...
		// template<class U, class V> pair (const std::pair<U,V>& pr):first(pr.first),second(pr.second){
		// };
			 ...
			 class Alloc = std::allocator<ft::pair<const Key,T> >    // multimap::allocator_type
				 > class multimap
				 {
					 public:
						 ...
						 size_type max_size() const{return (461168601842738790);}; // NOT YET
						 ...
						 ft::pair<iterator,bool> insert (const value_type& val){
							 ...
							 if (_size != 0)
							 {
								 ...
								 while (cpy->next != NULL)
								 {
									 if (comp_fun(val.first, cpy->data.first))
									 {
										 ret->next = cpy;
										 ret->prev = cpy->prev;
										 cpy->prev->next = ret;
						 ...
						 void erase (iterator position){
							 ...
							 if (position == this->end())
								 return ;
							 cpy = _head->next;
							 while (cpy->data.first != position->first)
								 cpy = cpy->next;
